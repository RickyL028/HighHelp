            ...
            <div class="mx-auto">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <div class="relative w-full md:w-96">
                        <input type="text" id="search-input" placeholder="Search essays..." class="w-full pl-10 pr-4 py-2 rounded-lg border border-gray-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" />
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                        ...
                        <tbody class="bg-white dark:bg-neutral-800 divide-y divide-gray-200 dark:divide-neutral-700">
                            {results?.length === 0 ? (
                                ...
                                results.map((e: any) => (
                                    <tr
                                        class={`search-item hover:bg-gray-50 dark:hover:bg-neutral-700 transition-colors cursor-pointer ${e.is_deleted ? 'bg-red-50 dark:bg-red-900/20' : ''}`}
                                        data-search-text={`${e.title} ${e.subject} ${e.question || ''}`}
                                        onclick={`window.location.href='/essays/view/${e.id}'`}
                                    >
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-neutral-400 local-date" data-timestamp={e.created_at}>
...
app.get('/essays/create', async (c) => {
    ...
    if (user.points < -2) {
        return c.html(
            <Layout title="Insufficient Points" user={user}>
                <div class="max-w-md mx-auto bg-white dark:bg-neutral-800 p-8 rounded-lg shadow-sm border border-gray-200 dark:border-neutral-700 text-center">
                    <div class="text-yellow-500 text-5xl mb-4">Ooops</div>
                    <h1 class="text-2xl font-bold mb-2 text-gray-900 dark:text-white">Insufficient Contribution</h1>
                    <p class="text-gray-600 dark:text-neutral-300 mb-6">You need a positive contribution to participate in this section. You currently have {user.points || 0} contributions.</p>
                    <p class="text-sm text-gray-500 dark:text-neutral-400 mb-6">Note the only purpose of this contribution system is to prevent over-dependence. Hence, we did not expect, or hope anyone seeing this prompt. Contribute by uploading resources, answering Q&As, or responding to others </p>
    ...
    return c.html(
        <Layout title="Submit Response" user={user}>
            <div class="max-w-2xl mx-auto bg-white dark:bg-neutral-800 p-8 rounded-lg shadow-sm border border-gray-200 dark:border-neutral-700">
                <h1 class="text-2xl font-bold mb-2 text-gray-900 dark:text-white">Submit Response</h1>
                <p class="text-gray-500 dark:text-neutral-400 mb-6 text-sm">Your submission will be anonymous.</p>
                ...
                <form action="/essays" method="post" enctype="multipart/form-data" class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" required placeholder="Your essay's title, or its topic e.g. Macbeth" class="w-full rounded-md border-gray-300 dark:border-neutral-600 shadow-sm p-3 border bg-white dark:bg-neutral-700 dark:text-white focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-neutral-300 mb-1">Question / Prompt (Required)</label>
                        <textarea name="question" required rows={3} placeholder="What was the question?" class="w-full rounded-md border-gray-300 dark:border-neutral-600 shadow-sm p-3 border bg-white dark:bg-neutral-700 dark:text-white focus:ring-blue-500 focus:border-blue-500"></textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-neutral-300 mb-1">Full Marks</label>
                        <input type="number" name="full_marks" placeholder="e.g. 20" class="w-full rounded-md border-gray-300 dark:border-neutral-600 shadow-sm p-3 border bg-white dark:bg-neutral-700 dark:text-white focus:ring-blue-500 focus:border-blue-500" />
                    </div>

                    <div class="p-4 bg-gray-50 dark:bg-neutral-900 border border-gray-200 dark:border-neutral-700 rounded-lg">
                        <label class="block text-sm font-bold text-gray-700 dark:text-neutral-300 mb-2">Essay Content</label>
                        <p class="text-xs text-gray-500 dark:text-neutral-400 mb-3">You can paste text OR upload a file (PDF/Docx).</p>

                        <div class="mb-4">
                            <label class="block text-xs font-bold text-gray-500 dark:text-neutral-400 mb-1">Option 1: Paste Text</label>
                            <textarea name="content" rows={6} placeholder="Paste your essay here..." class="w-full rounded-md border-gray-300 dark:border-neutral-600 shadow-sm p-3 border bg-white dark:bg-neutral-700 dark:text-white focus:ring-blue-500 focus:border-blue-500"></textarea>
                        </div>

                        <div>
                            <label class="block text-xs font-bold text-gray-500 dark:text-neutral-400 mb-1">Option 2: Upload File</label>
                            <input
                                type="file"
                                name="file"
            ...
            <div class="mx-auto">
                    ...
                    <div class="bg-white dark:bg-neutral-800 rounded-lg border border-gray-200 dark:border-neutral-700 divide-y divide-gray-100 dark:divide-neutral-700/50 shadow-sm">
                        {comments?.length === 0 ? (
                            <div class="p-8 text-center text-gray-500 dark:text-neutral-400">
                                No feedback yet. Be the first to respond!
                            </div>
                        ) : (
                                    ...
                                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-3">
                                        ...
                                        <div class="flex items-center gap-3 shrink-0">
                                            ...
                                            {(!comment.is_deleted && user && (canCommentModeration(user) || user.id === comment.author_id)) ? (
                                                <form action={`/essays/feedback/${comment.id}/delete`} method="post" class="m-0">
                                                    <button class="text-gray-400 hover:text-red-500 text-sm font-medium transition-colors" onclick="return confirm('Delete feedback?')">Delete</button>
                                                </form>
                                            ) : null}
                                        </div>
                                    </div>
                ...
                {user ? (
                    <div class="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-lg border border-blue-100 dark:border-blue-900/40">
                        <h3 class="text-lg font-bold text-blue-900 dark:text-blue-200 mb-4">Respond</h3>
